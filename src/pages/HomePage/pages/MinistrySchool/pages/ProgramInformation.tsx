...
const ProgramInformation = () => {
  const { currentApplicant, selectedCohort, selectedProgram } =
    useProgramsStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      userId: currentApplicant ? currentApplicant.user.user_id : null, // Add selected member's ID if available
    };
  ...
  const getAvailabilityBadge = (format: string) => {
    ...
    return (
      <span
        className={`${formatBadgeClass} text-xs font-medium px-2.5 py-0.5 rounded-full`}
      >
        {format === "In_Person"
          ? "In-Person"
          : format === "Hybrid"
    ...
    <div className="bg-white rounded-lg shadow-md w-full max-w-3xl max-h-[80vh] mx-auto overflow-y-scroll px-6 md:px-8">
      ...
      <div className="text-center mb-6 sticky top-0 bg-white z-10 p-4 rounded-lg w-[calc(100%+10px)] -mx-8 pt-8">
        ...
        <p className="text-gray-600">
          We found your membership information in our system.
        </p>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="font-medium">
              {currentApplicant.user.country_code +
                " " +
                currentApplicant.user.primary_number}
            </p>
          </div>
          <div>
          ...
          <div className="space-y-4">
            {currentApplicant.courses?.map((option: ClassOption) => {
              const percentage = Math.round(
                (option.enrolled / option.capacity) * 100
              );
                  ...
                  <div className="flex items-start">
                    <div className="flex-shrink-0 mt-0.5">
                      <input
                        ...
                        onChange={() =>
                          percentage < 100 && setSelectedClass(option.id)
                        }
                    ...
                    <div className="ml-3 flex-grow">
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        ...
                        <p>
                          ...
                          {option.enrolled}/{option.capacity} ({percentage}%
                          full)
                        </p>
                        {availableSpots > 0 && (
                          <p>
