    ...
    return (
        <div className="">
            <div className={`min-h-screen pt-4  fixed bg-white shadow-sm z-10 ${!show ? "w-10 min-w-[3vw]" : "w-[15%] min-w-[200px]"} transition-all duration-400 linear`} style={props.style}>
                ...
                {show?<div className='flex gap-x-4'><ChurchLogo show={show} /><div onClick={handleClick}><LogoutIcon/></div></div>:<div onClick={handleClick}><HoverSwitch/></div>}
                ...
                {items.map((item, index) => {
                    ...
                    return permissions["view_" + item["key"]] && (
                        <NavLink 
                            to={item["key"]} 
                            className={({ isActive }) => 
                                `hover:border-[#6539C3]  hover:border-l-2 transition h-10 z-10 flex items-center py-7 ${!show ? " justify-center" : "px-2 py-7 mx-2"} my-2 rounded-xl ${isActive ? "bg-[#6539C3] text-white shadow-lg" : "hover:text-primaryViolet"}`}
                            key={index}
                        >
