...
const EventRegister = () => {
  ...
  const handleFindMember = () => {
    ...
    axios
      .get(
        `${baseUrl}/event/search-user?phone=${memberDetails.phone_number}&country_code=${memberDetails.country_code}`
      )
      .then((res) => {
        setLoading(false);
        if (res.status == 200) {
  ...
  const handleConfirm = () => {
    setLoading(true);
    axios
      .post(`${baseUrl}/event/sign-attendance?event_id=${id}`, {
  ...
  const handleNewAttendee = () => {
    setLoading(true);
    axios
      .post(`${baseUrl}/event/sign-attendance?event_id=${id}`, memberDetails)
      .then(() => {
        setLoading(false);
        setMemberFound("confirmed");
      })
    ...
    <FormWrapper>
      ...
      {memberFound == "not_found" && (
        <RegisterMember
          ...
          name={name}
          handleChange={handleChange}
          onSubmit={handleNewAttendee}
        />
      )}
      {memberFound == "confirmed" && <MemberConfirmed />}
    </FormWrapper>
  );
};
