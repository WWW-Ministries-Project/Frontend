...
| `amount` | `INT` | minor units. What was **quoted** — the value the client sent to `/initialize` |
...
`amount_paid <> amount` silently skips exactly the rows most worth finding. The
correct query is:

```sql
WHERE amount_paid IS NULL OR amount_paid <> amount
```

Anyone using the naive form will miss the worst discrepancies without any error —
it just quietly returns fewer rows than it should.
